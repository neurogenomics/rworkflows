...
improvements: 1. Uses dynamic variables to specify R/Bioconductor
versions (e.g. `r: "latest"`) and the name of your R package, as opposed
to static names that are likely to become outdated
(e.g. `r: "4.0.1"`).  
2. Additional error handling and dependencies checks.  
3. Re-renders `README.Rmd` before rebuilding the documentation website.
